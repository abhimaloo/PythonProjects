...
Approach: Sweep Line / Min Heap
Key Idea:
Track current meetings using a min-heap of end times.
At each meeting start:
If it starts after or at the earliest ending meeting, reuse the room (pop from heap).
Otherwise, allocate a new room (push into heap).
The size of the heap tells you the number of rooms used at that time.
Track the maximum heap size during the sweep.
